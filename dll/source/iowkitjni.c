...
static jlong HandleToJLong(IOWKIT_HANDLE handle)
{
	if (sizeof(IOWKIT_HANDLE) == sizeof(jlong))
		return (jlong)handle;
	else
		return (jlong)((int)handle);
}

static IOWKIT_HANDLE JLongToHandle(jlong handle)
{
	if (sizeof(IOWKIT_HANDLE) == sizeof(jlong))
		return (IOWKIT_HANDLE)handle;
	else
...
(JNIEnv *env, jclass cl, jlong handle)
{
	...
	if (IowKitReadImmediate(JLongToHandle(handle), &read) != FALSE)
	{
		for (i = 0; i < 4; i++)
		{
